...
CONFIG_FLAGS="-T 12e0 -c 3 -w 0.25 -s 333 --no-sticky-boundary -P ChainConfigs/testConfig.chains"
...
OUT_DIR="Out_$(echo ${CONFIG_FLAGS} | flagsToNameFilter)_${SLURM_JOB_ID}"

# Settings for config data and shared libraries
REPO_BASE_DIR=${HOME}/Repo/active-microemulsion
REPO_ITEMS_TO_COPY=( "active-microemulsion" "lib" "ChainConfigs" "sequenceFigureBuilder.sh" ) # Will be copied with "cp -r"
...
for item in ${REPO_ITEMS_TO_COPY[@]}; do
    cp -r ${REPO_BASE_DIR}/${item} ${scratch}/.
done
